...
(async function () {
  ...
  let gotoInfo = "ok";
  try { await page.goto(POST, { waitUntil: "domcontentloaded", timeout: 45000 }); } catch (e) { gotoInfo = "gotoerr:" + String(e.message).slice(0, 150); }
  ...
  try { await page.screenshot({ path: OUT + "/probe8_anchor.png" }); } catch (e) {}
  fs.writeFileSync(OUT + "/probe8.json", JSON.stringify({ goto: gotoInfo, state: state }, null, 2));
  fs.writeFileSync(OUT + "/probe8_events.txt", ev.join("\n"));
  await ctx.close();
  await browser.close();
  console.log("PROBE8 DONE");
})().catch(function (e) { console.log("PROBE8 FAILED: " + e.message); process.exit(1); });
